...
- Send the "DELETE" request to the API: `response = request.delete(url)`

- According to the API we should be getting a `204` status code, to check that we are getting the right status code: `print(response.status_code)`

- Then can also verify the correct status code using the assert method: `assert response.status_code == 204`.
