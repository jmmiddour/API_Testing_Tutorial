import unittest
from unittest.mock import patch
from Unit_Testing_your_Code_YouTube.employee import Employee


class TestEmployee(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        """
        This method will run its code at the BEGINNING,
            BEFORE anything else gets ran.
        """
        print('setupClass')

    @classmethod
    def tearDownClass(cls):
        """
        This method will run its code at the END,
            AFTER ALL code has already been ran.
        """
        print('teardownClass')

    def setUp(self):
        """
        This method will run its code BEFORE every test is ran.
        """
        print('setUp')
        self.emp_1 = Employee('Corey', 'Schafer', 50000)
        self.emp_2 = Employee('Sue', 'Smith', 60000)

    def tearDown(self):
        """
        This method will run its code AFTER every test is ran.
        """
        print('tearDown\n')

    def test_email(self):
        """
        Method to test the email functionality
        """
        # This print is just to display the order in which the test are being ran.
        print('test_email')

        # Check that we are getting the expected values
        self.assertEqual(self.emp_1.email, 'Corey.Schafer@email.com')
        self.assertEqual(self.emp_2.email, 'Sue.Smith@email.com')

        self.emp_1.first = 'John'    # Change employee 1 first name
        self.emp_2.first = 'Jane'    # Change employee 2 first name

        # Check that the changes worked and getting the proper output
        self.assertEqual(self.emp_1.email, 'John.Schafer@email.com')
        self.assertEqual(self.emp_2.email, 'Jane.Smith@email.com')

    def test_fullname(self):
        """
        Method to test the fullname functionality
        """
        # This print is just to display the order in which the test are being ran.
        print('test_fullname')

        # Check that we are getting the expected values
        self.assertEqual(self.emp_1.fullname, 'Corey Schafer')
        self.assertEqual(self.emp_2.fullname, 'Sue Smith')

        self.emp_1.first = 'John'    # Change employee 1 first name
        self.emp_2.first = 'Jane'    # Change employee 2 first name

        # Check that the changes worked and getting the proper output
        self.assertEqual(self.emp_1.fullname, 'John Schafer')
        self.assertEqual(self.emp_2.fullname, 'Jane Smith')

    def test_apply_raise(self):
        """
        Method to test the apply_raise functionality
        """
        # This print is just to display the order in which the test are being ran.
        print('test_apply_raise')

        self.emp_1.apply_raise()     # Give employee 1 a raise
        self.emp_2.apply_raise()     # Give employee 2 a raise

        # Check that the raise took affect and getting the proper output
        self.assertEqual(self.emp_1.pay, 52500)
        self.assertEqual(self.emp_2.pay, 63000)

    def test_monthly_schedule(self):
        """
        Method to create a mocked object for testing purposes.
        """
        # Use patch as a context manager to mock that
        #   the server is currently running.
        with patch('employee.requests.get') as mocked_get:
            # Mocks the return_value.ok and assigns it to True,
            #   to mimic the server currently being up.
            mocked_get.return_value.ok = True
            # Mocks the return_value.text and assigns it to 'Success'
            mocked_get.return_value.text = 'Success'

            # Create a test schedule
            schedule = self.emp_1.monthly_schedule('May')
            # Check if the correct url was called
            mocked_get.assert_called_with('http://company.com/Schafer/May')
            # Verify the results are outputted properly
            self.assertEqual(schedule, 'Success')

            # Mocks the return_value.ok and assigns it to False,
            #   to mimic the server being down
            mocked_get.return_value.ok = False

            # Create another test schedule
            schedule = self.emp_2.monthly_schedule('June')
            # Check if the correct url was called
            mocked_get.assert_called_with('http://company.com/Smith/June')
            # Verify the results are outputted properly
            self.assertEqual(schedule, 'Bad Response!')


if __name__ == '__main__':
    unittest.main()